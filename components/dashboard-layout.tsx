...
const infraNavigation = [
  ...
  { id: 'volumeattachments', label: 'Volume Attachments', icon: Paperclip, href: '/i/volumeattachments', section: 'config' },
...
export function DashboardLayout({ children, onChatToggle, chatVisible, chatLayoutMode, chatWidth, mode }: DashboardLayoutProps) {
  const [sidebarOpen, setSidebarOpen] = useState(true)
  const [sidebarCompact, setSidebarCompact] = useState(false)
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(['workloads', 'config', 'network', 'rbac', 'cluster', 'cloud', 'blockchain']))
  ...
  return (
    <div className="flex h-screen bg-background">
      {/* Sidebar */}
      <div className={`${sidebarOpen ? (sidebarCompact ? 'w-16' : 'w-64') : 'w-0'} transition-all duration-300 overflow-hidden bg-card border-r flex flex-col h-screen`}>
        ...
        {/* Context Switcher */}
        {sidebarCompact ? (
          <ContextSwitcher variant="compact" />
        ) : (
          <ContextSwitcher />
        )}
        ...
        <nav className="flex-1 mt-4 pb-4 overflow-y-auto">
          {navigationItems.map((item, index) => {
            ...
            const isFirstInSection = sidebarCompact && item.section && 
              (!prevItem || prevItem.isHeader || prevItem.section !== item.section)
            ...
            return (
              <div key={item.id}>
                ...
                <NextLink
                  href={item.href}
                  className={`w-full flex items-center ${sidebarCompact ? 'justify-center px-2 py-3' : 'px-4 py-2'} text-sm hover:bg-accent transition-colors ${
