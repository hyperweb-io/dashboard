      ...
      >
        ...
        <nav className='flex-1 mt-8 pb-4 overflow-y-auto'>
          {navigationItems.map((item) => {
            ...
            // Skip items without href or icon (headers shouldn't reach here but safety check)
            if (!item.href || !Icon) {
              return null;
            }
        ...
        <div className='p-4 border-t border-border text-center text-xs text-muted-foreground'>
          Built with <Heart className='inline-block w-3 h-3 mx-1 text-red-500 fill-red-500' /> by
          Hyperweb
        </div>
      </div>
      ...
      <div className='flex-1 flex flex-col'>
        ...
        <header className='bg-card border-b px-6 py-4 flex items-center justify-between'>
          <div className='flex items-center'>
            ...
            <h2 className='ml-4 text-xl font-semibold'>{activeSection}</h2>
          </div>
